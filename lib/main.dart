...
const Color kHistoryColor = Color(0xFF2C2C2C); // Fundo do histórico
const Color kDisplayTextColor = Colors.white; // Cor do texto do buffer
const Color kErrorTextColor = Colors.redAccent; // Cor do texto de erro

// Botões 'Outlined' (Geral e Científico)
const Color kOutlinedButtonBg = Color(0xFF3A3A3A); // Fundo escuro do botão
const Color kOutlinedButtonText = Color(0xFF8AB4F8); // Texto em azul néon claro
const Color kOutlinedButtonTextDefault = Colors.white70; // Texto dos números

// Botão 'Elevated' (Apenas o '=' )
const Color kElevatedButtonBg = Color(0xFF4285F4); // Azul vibrante para o igual
const Color kElevatedButtonText = Colors.white; // Texto branco
...
class CalculatorEngine extends StateNotifier<CalculatorState> {
  ...
  void evaluate() {
    try {
      final parser = Parser(); // Usando Parser de math_expressions
...
final buttonDefinitions = <ButtonDefinition>[
  ...
  ButtonDefinition(
    areaName: 'e',
    op: (engine) =>
        engine.addToBuffer('e'), // Removido '(', pois 'e' é constante
    label: 'e',
  ),
  ButtonDefinition(
    areaName: 'ln',
...
class CalculatorApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        body: Container(
          color: kBackgroundColor, // Fundo principal
          child: SafeArea(
            child: LayoutGrid(
              areas: '''
              ...
              children: [
                      ...
                      child: state.error.isEmpty
                          ? AutoSizeText(
                              ...
                              style: TextStyle(
                                fontSize: 80,
                                color:
                                    kDisplayTextColor, // Cor do texto do buffer
                              ),
                              maxLines: 2,
                            )
                          : AutoSizeText(
                              ...
                              style: const TextStyle(
                                fontSize: 80,
                                color: kErrorTextColor, // Cor do texto de erro
                              ),
                              maxLines: 2,
                            ),
                    ),
                ...
                NamedAreaGridPlacement(
                  areaName: 'history',
                  child: Container(
                    color: kHistoryColor, // Fundo do histórico
                    child: Padding(
                          ...
                          ...state.calcHistory.map(
                            (result) => ListTile(
                              title: Text(
                                ...
                                style: const TextStyle(
                                  color: Colors
                                      .white70, // Cor dos itens do histórico
                                ),
                              ),
                            ),
                          ),
...
class CalcButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final buttonStyle = switch (type) {
      CalcButtonType.elevated => ElevatedButton.styleFrom(
        backgroundColor: kElevatedButtonBg, // Cor de fundo do '='
        foregroundColor: kElevatedButtonText, // Cor do texto do '='
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
        padding: const EdgeInsets.all(0),
      ),
      _ => OutlinedButton.styleFrom(
        backgroundColor: kOutlinedButtonBg, // Cor de fundo dos outros botões
        foregroundColor:
            kOutlinedButtonText, // Cor padrão do texto (científico/operador)
        side: BorderSide.none, // Remove a borda padrão do OutlinedButton
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
        padding: const EdgeInsets.all(0),
      ),
    };

    // Função construtora para criar o botão com o estilo determinado
    final buttonConstructor =
        ({required Widget child, required VoidCallback onPressed}) {
          return switch (type) {
            CalcButtonType.elevated => ElevatedButton(
              style: buttonStyle,
              onPressed: onPressed,
              child: child,
              autofocus: false,
    ...
    // Determina a cor final do texto do botão
    Color textColor;
    if (type == CalcButtonType.elevated) {
      textColor = kElevatedButtonText;
    } else if (int.tryParse(label) != null || label == '.' || label == '0') {
      textColor = kOutlinedButtonTextDefault; // Números em branco suave
    } else {
      textColor = kOutlinedButtonText; // Funções e operadores em azul néon
    }
